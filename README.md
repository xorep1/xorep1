<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&height=250&color=0:4F46E5,50:7C3AED,100:A855F7&text=Mohsen%20Tavallai&fontSize=50&fontColor=ffffff&animation=fadeIn&fontAlignY=38" />
</p>

<p align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Inter&weight=600&size=26&duration=3000&pause=1000&color=A855F7&center=true&vCenter=true&width=900&lines=Full-Stack+Developer;AI+%2F+ML+Enthusiast;Building+Scalable+Systems;Clean+Code+Advocate" />
</p>

<p align="center">
  <img src="https://img.shields.io/badge/Architecture-Clean%20Code%20%26%20OOP-7C3AED?style=for-the-badge" />
  <img src="https://img.shields.io/badge/Specialization-Full--Stack%20Development-6366F1?style=for-the-badge" />
  <img src="https://img.shields.io/badge/Focus-Scalable%20Systems-A855F7?style=for-the-badge" />
</p>

<p align="center">
  <img src="https://img.shields.io/badge/Location-Iran-4F46E5?style=for-the-badge&logo=googlemaps&logoColor=white" />
</p>

<p align="center">
  <a href="https://your-portfolio.com">
    <img src="https://img.shields.io/badge/Portfolio-Visit-7C3AED?style=for-the-badge&logo=vercel&logoColor=white"/>
  </a>
  <a href="https://linkedin.com/in/your-profile">
    <img src="https://img.shields.io/badge/LinkedIn-Connect-6366F1?style=for-the-badge&logo=linkedin&logoColor=white"/>
  </a>
  <a href="mailto:your@email.com">
    <img src="https://img.shields.io/badge/Email-Contact-A855F7?style=for-the-badge&logo=gmail&logoColor=white"/>
  </a>
  <a href="https://github.com/yourusername">
    <img src="https://img.shields.io/badge/GitHub-Follow-4F46E5?style=for-the-badge&logo=github&logoColor=white"/>
  </a>
</p>

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=yourusername&style=for-the-badge&color=7C3AED" />
  <img src="https://img.shields.io/github/followers/yourusername?style=for-the-badge&color=6366F1" />
  <img src="https://img.shields.io/github/stars/yourusername?style=for-the-badge&color=A855F7" />
</p>

---

# About

I am a Full-Stack Developer focused on building scalable, secure, and high-performance software systems. My expertise spans across modern frontend architectures, robust backend development, and AI integrations.

My work combines strong software engineering principles—like **OOP** and **Clean Code**—with modern tools. I enjoy designing products that are technically solid, operationally reliable, and capable of delivering measurable business impact.

### Open To

- Full-Stack Development
- Backend Engineering (Python/Node.js)
- Frontend Development (React Ecosystem)
- Technical Consulting

---

# Tech Stack

## Languages

<p align="center">
<img src="https://skillicons.dev/icons?i=python,js,ts,html,css" />
</p>

## Frontend Frameworks

<p align="center">
<img src="https://skillicons.dev/icons?i=react,nextjs,tailwind,materialui,redux,sass" />
</p>

## Backend & APIs

<p align="center">
<img src="https://skillicons.dev/icons?i=fastapi,django,express,nodejs" />
</p>

## Databases & Message Brokers

<p align="center">
<img src="https://skillicons.dev/icons?i=postgres,mysql,sqlite,redis,rabbitmq" />
</p>

## DevOps, OS & Tooling

<p align="center">
<img src="https://skillicons.dev/icons?i=docker,linux,git,bash" />
</p>

---

# Libraries, Tools & Concepts

Beyond the main frameworks, I actively use a rich ecosystem of libraries and architectural patterns to build robust applications:

| Category | Technologies & Skills |
|----------|-----------------------|
| **Frontend Tools** | React Native, Shadcn UI, Formik, React Hook Form, React Router, TanStack (Query/Table), Axios, Alpine.js |
| **Backend & Data** | Pydantic, ORMs, SQL |
| **AI & LLMs** | Ollama |
| **Architecture** | Object-Oriented Programming (OOP), Clean Code |

---

# Featured Projects

<details>
<summary><b>Scalable Full-Stack Application</b></summary>

### Overview

High-performance application designed for seamless user experience and robust data handling.

| Metric | Value |
|----------|----------|
| Stack | Next.js, FastAPI, PostgreSQL, Redis |
| Architecture | Clean Code, OOP |
| State Management | Redux, TanStack |
| Impact | Improved loading times and data consistency |
| Repository | Public/Private |

#### Details

Built a modern web platform integrating a reactive frontend with a highly optimized Python backend, utilizing Redis for caching and RabbitMQ for background tasks.

</details>

<details>
<summary><b>AI-Integrated Platform</b></summary>

### Overview

Intelligent application leveraging local LLMs for data processing.

| Metric | Value |
|----------|----------|
| Stack | Django, React, Ollama, Docker |
| Infrastructure | Linux, SQLite/MySQL |
| Data Validation | Pydantic |
| Impact | Automated text processing workflows |
| Repository | Private |

#### Details

Developed a seamless interface between a modern web frontend and local AI models using Ollama, focusing on secure and fast API responses.

</details>

---

# GitHub Analytics

<p align="center">
<img height="180em" src="https://github-readme-stats.vercel.app/api?username=yourusername&show_icons=true&theme=tokyonight&hide_border=true"/>
<img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=yourusername&layout=compact&theme=tokyonight&hide_border=true"/>
</p>

<p align="center">
<img src="https://github-readme-streak-stats.herokuapp.com/?user=yourusername&theme=tokyonight&hide_border=true"/>
</p>

---

# Contribution Activity

<p align="center">
<img src="https://github-readme-activity-graph.vercel.app/graph?username=yourusername&theme=tokyo-night&hide_border=true"/>
</p>

---

# Current Focus

```yaml
Learning:
  - Advanced Distributed Systems
  - Local AI Integrations (Ollama)
  - Microservices with RabbitMQ & Redis

Building:
  - Cross-platform apps (React Native)
  - High-performance APIs (FastAPI & Express)
  - Interactive UIs (Next.js & Shadcn)

Exploring:
  - Advanced State Management (TanStack)
  - Clean Architecture Patterns

Open_To:
  - Full-Stack Roles
  - Backend Architecture
  - Open Source Collaboration
```

# Connect



<p align="center">

<a href="mohsentavalay@Gmail.com">

<img src="https://img.shields.io/badge/Gmail-Contact-A855F7?style=for-the-badge&logo=gmail&logoColor=white"/>

</a>



<a href="https://www.linkedin.com/in/tavallai">

<img src="https://img.shields.io/badge/LinkedIn-Network-6366F1?style=for-the-badge&logo=linkedin&logoColor=white"/>

</a>



<a href="https://github.com/xorep1">

<img src="https://img.shields.io/badge/GitHub-Follow-4F46E5?style=for-the-badge&logo=github&logoColor=white"/>

</a>



<a href="https://tavallai.ir">

<img src="https://img.shields.io/badge/Portfolio-Visit-7C3AED?style=for-the-badge&logo=vercel&logoColor=white"/>

</a>

</p>



---




<p align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&section=footer&height=120&color=0:4F46E5,50:7C3AED,100:A855F7"/>

</p>
